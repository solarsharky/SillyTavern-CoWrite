...
> 当前版本：`v0.1.0-beta.7`。这是公开测试版，请先备份重要记录再更新。
...
Char 先准备一组混合题型，User 手动填写；下一轮 Char 给出自己的答案，双方再评价彼此。题目数量、顺序和每一阶段的行为都可以在格式分类中修改。
...
历史聊天条数、当前激活世界书、手选世界书条目、长记录预算和默认连接统一放在“设置”，对所有分类、新记录、继续生成和下一卷生效。当前激活世界书通过 SillyTavern 的 `world_info_before` / `world_info_after` 位置读取；手选条目会在发送前再次排除停用或已删除条目。上下文只供模型参考，响应仍只写入共笔记录。

生成过程中窗口右下角始终显示“停止本轮”。停止后即使服务端稍后返回内容，插件也会丢弃该响应，不保存半成品。

核心 JSON 协议默认只读。高级模式允许在确认风险后覆盖协议，但结构校验、User 内容保护、引用校验和追加事务不能关闭。

记录超过 `min(12,000 tokens, ST 最大上下文的 40%, 全局长记录预算)` 后，共笔会先为较早轮次生成滚动摘要；后续请求仅发送摘要、最近三轮及尚未回答/评价的输入，完整历史仍保存在记录文件中。摘要失败时不会继续主生成，可以重试或创建“下一卷”。
